...
@section("content")
    <div class="row">
        <div class="card col-sm-12 col-md-12 col-lg-8">
            ...
            <div class="card-body">
                <form action="{{route("cms.home-page-management.update",$section->id)}}" method="post"
                      enctype="multipart/form-data">
                    @csrf
                    @method("PUT")
                    <label for="title">Kısım Başlığı</label>
                    <input type="text" name="title" id="title" placeholder="Başlık" value="{{$section->title}}">
                    <label for="content"></label>
                    <label for="contentText">İçerik</label>
                    <textarea name="content_text" id="contentText" cols="30" rows="10">
                        {{$section->content}}
                    </textarea>
                    ...
                    <input type="text" name="video_link" id="video_link" value="{{$section->video_link}}">
                    <label for="image_link">Resim Linki</label>
                    <input type="text" name="image_link" id="image_link" value="{{$section->image_link}}">
                    <label for="page_id">Yönlendirilecek Sayfa</label>
                    <select name="page_id" id="page_id">
                        <option value="">Yok</option>
                        @foreach($pages as $item)
                            <option value="{{$page->id}}" @if($section->page_id==$item->id) selected @endif >
                                {{$page->title}}
                            </option>
                        @endforeach
                    </select>
